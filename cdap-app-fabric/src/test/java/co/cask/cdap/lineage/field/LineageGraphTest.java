...
public class LineageGraphTest {
  ...
  public void testInvalidGraph() throws Exception {
    ...
    try {
      ...
      LineageGraph graph = new LineageGraph(operations);
      Assert.fail("Graph creation should fail since no read operation is specified.");
    } catch (IllegalArgumentException e) {
      String msg = "Field level lineage graph requires at least one operation of type 'READ'.";
    ...
    try {
      ...
      String msg = "Field level lineage graph requires at least one operation of type 'WRITE'.";
      Assert.assertTrue(e.getMessage().equals(msg));
    }
  ...
  public void testValidGraph() throws Exception {
    ...
    operations.add(write);
    operations.add(parse);
    LineageGraph graph1 = new LineageGraph(operations);

    // Serializing and deserializing the graph should result in the same checksum.
    String graphJson = GSON.toJson(graph1);
    LineageGraph graph2 = GSON.fromJson(graphJson, LineageGraph.class);
    ...
    LineageGraph graph3 = new LineageGraph(operations);
    ...
    operations.add(parse);
    operations.add(read);
    LineageGraph graph4 = new LineageGraph(operations);
